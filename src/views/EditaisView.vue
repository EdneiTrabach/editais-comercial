    ...
    <div class="modal-content">
      ...
      <form @submit.prevent="handleAddRepresentante" class="form-grid">
        ...
        <div class="modal-actions">
          <button type="button" 
                  class="btn-cancelar" 
                  @click="showRepresentanteModal = false">
            Cancelar
          </button>
          <button type="submit" class="btn-salvar">
            Salvar
...
const handleSubmit = async () => {
  try {
    ...
    const processoData = {
      ...
      responsavel: user.id, // ID do usuário autenticado
...
input, select, textarea {
  ...
  transition: all 0.3s ease;
  background: #f8f9fa;
  font-family: 'JetBrains Mono', monospace; /* Garantindo a fonte aqui também */
  font-size: 0.9rem; /* Ajuste o tamanho se necessário */
}
...
.btn-add-representante {
  ...
  min-width: fit-content; /* Garante espaço para o texto */
}
